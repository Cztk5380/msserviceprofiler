...
public:
    explicit DbExecutor(const msptiActivityMarker &activity) : kind(activity.kind), flag(activity.flag),
        sourceKind(activity.sourceKind), timestamp(activity.timestamp), id(activity.id),
        objectId(activity.objectId), name(activity.name)
    {}
