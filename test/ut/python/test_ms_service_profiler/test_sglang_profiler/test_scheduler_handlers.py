...
class TestSchedulerHandlers:
    ...
    @pytest.mark.parametrize(
        ("is_decode", "is_extend", "expected"),
        [(True, False, "decode"), (False, True, "prefill"), (False, False, "unknown")],
    )
    ...
    def test_get_next_batch_with_batch(self):
        ...
        mock_batch = MockBatch(
            reqs=[mock_req], forward_mode=MockForwardMode(is_decode=True)
        )
        mock_original.return_value = mock_batch

        mock_scheduler = Mock()
        mock_scheduler.is_hybrid = False
        mock_scheduler.is_hybrid_swa = None
    ...
    def test_run_batch_execution(self):
        ...
        mock_batch = MockBatch(
            reqs=[mock_req], forward_mode=MockForwardMode(is_extend=True)
        )
    ...
    def test_queue_operations(self):
        ...
        result = scheduler_handlers.add_request_to_queue(
            mock_original, mock_scheduler, mock_req, False
        )
