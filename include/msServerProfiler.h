...
public:
    using AttrCollectCallback = void (*)(TCollector *pCollector, T pParam);
};

class CollectorHelper {
public:
    template <typename T>
    inline void AddNumArrayAttr(const char *attrName, const T &startIter, const T &endIter)
    {
    ...
    template <typename T>
    void AddArrayAttr(const char *attrName, const T &startIter, const T &endIter,
...
public:
    ...
    inline ProfilerBase &AddNumArrayAttr(const char *attrName, const T &startIter, const T &endIter)
    {
    ...
    template <Level levelAttr = level, typename T>
    ProfilerBase &AddArrayAttr(const char *attrName, const T &startIter, const T &endIter,
        typename ArrayCollectorHelper<CollectorHelper, T>::AttrCollectCallback callback)
    {
        if (IsEnable(levelAttr)) {
    ...
    template <Level levelAttr = level>
    inline ProfilerBase &AddAttr(const char *attrName, const std::string &value)
    {
        if (IsEnable(levelAttr)) {
            collector_.AddAttr(attrName, value);
        }
...
public:
    Span(const char *spanName, const ResID &rid, bool autoStart = true, bool autoEnd = true) : autoEnd_(autoEnd)
    {
