...
import pandas as pd
import numpy as np
from ms_serviceparam_optimizer.inference.dataset import CustomLabelEncoder, preset_category_data
...
class TestMyDataSet(unittest.TestCase):
    ...
    def test_analysis_batch_feature(self, mock_plot, mock_save):
        ...
        ds = MyDataSet()
        ds.load_data = pd.DataFrame({"batch_size": (1, 2, 3), "other_col": ("a", "b", "c")})
    ...
    def test_construct_data_shape_mismatch(self, mock_logger):
        """测试特征和标签维度不匹配的情况"""
        ds = MyDataSet()
        with patch.object(ds, 'preprocess_dispatch', return_value=(pd.DataFrame(), pd.DataFrame([1, 2]))):
...
class TestPlotCustomPairplot(unittest.TestCase):
    ...
    def test_saving_to_file(self):
        ...
        MyDataSet.plot_custom_pairplot(df=self.sample_df, middle_save_path=self.temp_dir, file_name=file_name)
...
def feature_csv(tmpdir):
    ...
    with open(_feature_file, "w", encoding="utf-8") as f:
        f.write(
            """
"('batch_stage', 'batch_size', 'total_need_blocks', 'total_prefill_token', 'max_seq_len',"""
...
@pytest.fixture(name="all_feature_csv")
def fixture_all_feature_csv(tmpdir):
    _feature_file = Path(tmpdir).joinpath("feature.csv")
    with open(_feature_file, "w", encoding="utf-8") as f:
        _txt = (
            '''"('batch_stage', 'batch_size', 'total_need_blocks', 'total_prefill_token', 'max_seq_len','''
        ...
        _txt_3 = (
            '''"('op_name', 'call_count', 'input_count', 'input_dtype', 'input_shape', 'output_count','''
            ''' 'output_dtype', 'output_shape', 'host_setup_time', 'host_execute_time', 'kernel_execute_time','''
            ''' 'aic_cube_fops', 'aiv_vector_fops')"'''
        )
        _txt_4 = (
            '''"('total_param_num', 'total_param_size', 'embed_tokens_param_size_rate','''
        ...
        _txt_5 = (
            '''"('architectures', 'hidden_act', 'initializer_range', 'intermediate_size','''
        ...
        _txt_6 = (
            '''"('cache_block_size', 'mindie__max_seq_len', 'world_size', 'cpu_mem_size', 'npu_mem_size','''
            ''' 'max_prefill_tokens', 'max_prefill_batch_size', 'max_batch_size')"'''
        )
        _txt_7 = (
            '''"('atb_llm_razor_attention_enable', 'atb_llm_razor_attention_rope','''
        ...
        _txt_2 = '''"(('1396.0', '11', '0'),)"'''
        _txt_3 = (
            '''"(('LinearOperation', '32', '2', 'float16;float16', '1396,4096;6144,4096', '1', 'float16','''
            ''' '1396,6144', '158.13848484848492', '343.5108080808081', '296.228', '75587665920.0', '0.0'), )"'''
        )
        _txt_4 = (
            '''"('206', '8030265344', '0.06542008881344383', '0.16713984189860415', '0.7019873359741374','''
            ''' '1.632225018541056e-05', '1.632225018541056e-05', '5.1007031829408e-07', '0.06542067874312553')"'''
        )
        _txt_5 = (
            '''"(('LlamaForCausalLM',), 'silu', 0.02, 14336, 8192, 'llama', 32, 32, False, 'float16','''
