...
|Atlas 推理系列产品|  x   |
|Atlas 训练系列产品|  x   |
工具支持型号与SGLang框架服务化推理部署支持的NPU型号保持一致，具体可参考 [SGLang installation with NPUs support](https://docs.sglang.io/platforms/ascend/ascend_npu.html)。
...
2. 完成 SGLang 在NPU上的安装和部署，确保可以正常运行推理服务，具体请参见 [SGLang installation with NPUs support](https://docs.sglang.io/platforms/ascend/ascend_npu.html)。
...
vim /usr/local/pythonx.xx.xx/lib/pythonx.xx/site-packages/sglang/launch_server.py # 其中/usr/local/pythonx.xx.xx/lib/pythonx.xx/site-packages为pip show sglang回显的sglang安装路径
