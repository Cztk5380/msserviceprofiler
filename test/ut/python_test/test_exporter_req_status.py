...
def test_parse_valid_data(tmpdir, sample_data):
    """创建SQLite数据库"""
    # with patch('visual_db_fp')
    # global visual_db_fp
    # visual_db_fp = ""
    # create_sqlite_db(tmpdir)

    """测试export"""
    ExporterReqStatus.initialize({})
    ExporterReqStatus.export(sample_data)

    # db_fp = tmpdir / 'profiler.db'
    db_fp = 'profiler.db'
    ...
    assert sample_data["metric_data_df"].rename(columns={"start_datetime":"timestamp"}).equals(res)
