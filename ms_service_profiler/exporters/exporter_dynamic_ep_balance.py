...
TOP_K_OUTPUT_NAME_EXPERT = "top_{}_hot_map_by_expert.png"
TOP_K_OUTPUT_NAME_RANK = "top_{}_hot_map_by_rank.png"
NAME = "expert_hot"
    ...
    def export(cls, data: Dict) -> None:
        ...
        for instance_name, expert_hot_per_instance in expert_hot.items():
            for eplb_iteration, expert_hot_per_eplb in enumerate(expert_hot_per_instance):
                ...
                # shape: rank * expert_per_rank * layer -> total_expert * layer
                expert_hot_summed_expert = expert_hot_per_instance.reshape(-1, expert_hot_summed_expert.shape[-1])
                summed_hot_expert_output_path = os.path.join(output,
                                                             SUMMED_OUTPUT_NAME_EXPERT.format(instance_name))
...
def draw_hot_map_from_arr(arr, title="", x_label="", y_label="", output_path="hot_map.png"):
    ...
    plt.xticks(ticks=[i for i in range(arr.shape[0])], labels=list(range(arr.shape[0])))
    plt.yticks(ticks=[i for i in range(arr.shape[1])], labels=list(range(arr.shape[1])))
    plt.tight_layout()
